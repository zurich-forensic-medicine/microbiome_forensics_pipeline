...
x1 <- file.path(project_path, 'src/pipeline_steps/1_cut_adapters.sh')   # script to run
x <- paste(x1, raw_path)
cat(x)  # sanity check


# run bash command
# NOTE: first path is a path to Python in conda env, the second is to cutadapt itself
